...
class FizzBuzzGame(tk.Tk):
    ...
    def create_widgets(self):
        ...
        # row 0: Score label, Progressbar, Timer label
        ...
        self.timer_bar.step(99.99)
        self.timer_bar.grid(row=0, column=1, padx=10, pady=10)

        self.timer_label = ttk.Label(self, text="Time: " + str(self.timer))
        self.timer_label.grid(row=0, column=2, padx=10, pady=10)

        # row 1: Number label
        self.number_label = ttk.Label(self, text="Number: " + str(self.number))
        self.number_label.grid(row=1, column=1, padx=10, pady=10)

        # row 2: Fizz / Buzz / FizzBuzz buttons
        self.fizz_button = ttk.Button(self, text="Fizz", command=lambda: self.button_clicked(clicked="fizz"))
        self.fizz_button.grid(row=2, column=0, padx=10, pady=10)

        self.buzz_button = ttk.Button(self, text="Buzz", command=lambda: self.button_clicked(clicked="buzz"))
        self.buzz_button.grid(row=2, column=1, padx=10, pady=10)

        self.fizzbuzz_button = ttk.Button(self, text="FizzBuzz", command=lambda: self.button_clicked(clicked="fizzbuzz"))
        self.fizzbuzz_button.grid(row=2, column=2, padx=10, pady=10)

    def show_welcome_message(self):
        message = "Welcome to the FizzBuzz Game! You'll have %d seconds to get the highest score you can. " % self.timer + \
